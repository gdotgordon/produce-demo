...
- To start the server, run `docker-compose up` or `docker-compose up -d` to run in detached mode.  In the latter case, you can use a tool like *Kitematic* or use `docker logs` with the contianer name as such:
...
HTTP return codes:
- 200 (OK) for requests with mutliple produce items, but mixed results storing them
...
Note that two of the items are invald (one bad code, one bad name, plus we've sent the same item twice.  The response is:
```
[
    {
        "code": "B12T-4GH7-QPL9-3N4M",
